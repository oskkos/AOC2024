...
def part_one(lines: list[str]) -> int:
    ...
    Historians continue their search of this historically significant facility, an Elf operating
    a very familiar printer beckons you over.
    ...
    very specific order. The notation X|Y means that if both page number X and page number Y are
    to be produced as part of an update, page number X must be printed at some point before page
    number Y.
    ...
    - 61 is correctly in the middle because 75 and 47 are before it (75|61 and 47|61) and 53
      and 29 are after it (61|53 and 61|29).
    ...
    correct_updates, _ = get_correct_and_incorrect_updates(lines)

    result = 0
    for update in correct_updates:
        result += get_middle_page_number(update)
...
def part_two(lines: list[str]) -> int:
    ...
    """
    _, incorrect_updates = get_correct_and_incorrect_updates(lines)

    result = 0
    for incorrect_update in incorrect_updates:
        result += get_middle_page_number(incorrect_update)
...
def get_rules_and_updates(
    lines: list[str],
) -> tuple[dict[str, dict[str, bool]], list[list[str]], dict[str, dict[str, bool]]]:
    """
...
def get_correct_and_incorrect_updates(
    ...
    """
    Categorizes updates into correct and incorrect based on given rules.
    """

    rules, updates, values = get_rules_and_updates(lines)
    ...
    for update in updates:
        ...
        if correct:
            ...
            incorrect_updates.append(sort_update(update, values))
    return correct_updates, incorrect_updates


def get_middle_page_number(update: list[str]) -> int:
    """
    Given a list of strings representing page numbers, returns the middle page number as an integer.
    """
    return int(update[len(update) // 2])


def sort_update(update: list[str], values: dict[str, dict[str, bool]]) -> list[str]:
    """
