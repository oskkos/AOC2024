...
def part_one(lines: list[str]) -> int:
    ...
    for line in lines:
        ...
        left.append(int(row_numbers[0]))
        right.append(int(row_numbers[1]))
    left.sort()
    right.sort()

    for i, left_value in enumerate(left):
        total += abs(int(left_value) - int(right[i]))
    return total
...
def part_two(lines: list[str]) -> int:
    ...
    appear in both lists! Maybe the other numbers aren't location IDs at all but rather 
    ...
    for left_value in left:
        total += left_value * right.count(left_value)
    return total
